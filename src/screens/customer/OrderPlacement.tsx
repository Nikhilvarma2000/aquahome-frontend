...
//   }, [productId]);
  ...
  const handlePlaceOrder = async () => {
   ...
   console.log("✅ Final Order ID passed to Razorpay:", aquahomeOrderId);
      ...
      const paymentRes = await api.post('/payments/generate-order', {
        order_id: aquahomeOrderId
      });

      const { razorpay_order_id, amount, currency, key } = paymentRes.data;
      ...
      RazorpayCheckout.open(options).then(async (paymentResult) => {
        ...
        const verifyPayload = {
          ...
          aquahome_order_id: aquahomeOrderId
        };
